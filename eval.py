...
def task_exp(args, extra_args):
    ...
    if not args.loadCache:
        (X, regret_dict, title, xlabel, ylabel) = utils.task_exp(N_EXPS, N_TASKS, N_ARMS, HORIZON, OPT_SIZE, N_EXPERT, **extra_args)
    else:
        X = np.arange(N_TASKS)
...
def horizon_exp(args, extra_args):
    ...
    if not args.loadCache:
        ...
        utils.horizon_exp(N_EXPS, n_tasks, N_ARMS, OPT_SIZE, N_EXPERT, 
                        horizon_list = horizon_list, **extra_args)
    else:
...
def subset_exp(args, extra_args):
    ...
    if not args.loadCache:
        (X_e, regret_dict_e, title, xlabel, ylabel) = utils.subset_exp(N_EXPS, 
                                  ...
                                  opt_size_list = np.arange(1,N_ARMS+1,1), **extra_args)
    else:
        gap = extra_args['gap_constrain']
...
def arms_exp(args, extra_args):
    ...
    if not args.loadCache:
        ...
        utils.arms_exp(N_EXPS, N_TASKS, HORIZON, OPT_SIZE, N_EXPERT, n_arms_list, **extra_args)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--expArgs', help='arguments for horizon or arms. Example: (a,b,c) => range(a,b,c)', type=str, default=None)
    ...
    if args.loadCache is True:
        if args.nTasks != 1000 or args.nArms != 5 or args.nExps != 10 or args.optSize != 2 or args.horizon != 250 or args.expArgs is not None:
            assert False, "When using loadCache, please use the default setting for nTasks, nArms, nExps, optSize, and horizon."
        nExperts = 10
    else:
